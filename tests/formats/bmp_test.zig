...
test "Write a v4 bitmap when writing an image with bgr24 pixel format" {
    ...
    var stream_source_buffer: [1024]u8 = @splat(0); const stream_source_file_reader = read_file.reader(&stream_source_buffer); var stream_source = stream_source_file_reader.interface;
